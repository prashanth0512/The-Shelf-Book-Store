document.addEventListener('DOMContentLoaded', () => {
  ...
  // ─── SCROLL REVEAL & STATS COUNTERS SYSTEM ───────────────────
    ...
    function updateCounter(currentTime) {
      ...
      if (progress < 1) {
        ...
        if (target >= 1000) {
          ...
          counterEl.textContent = target + '+'; // Ensure '+' is added for values like '300+' and '50+' as well
        }
      }
    }
    requestAnimationFrame(updateCounter);
  ...
  // Expose toast to global window context for inline events
  ...
  if (newsletterFormMain) {
    newsletterFormMain.addEventListener('submit', (e) => {
      ...
      setTimeout(() => {
        showToast('Thank you! Welcome to The Shelf bookstore newsletter.');
  ...
  if (newsletterFormFooter) {
    newsletterFormFooter.addEventListener('submit', (e) => {
      ...
      setTimeout(() => {
        showToast('Successfully subscribed! Check your inbox for reading essays.');
        if (emailInput) emailInput.value = '';
        submitBtn.disabled = false;
        submitBtn.innerHTML = originalText;
      }, 1200);
