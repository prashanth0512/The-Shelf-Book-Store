document.addEventListener('DOMContentLoaded', () => {
  ...
  const isLightDefault = document.body.classList.contains('light-mode');
  const savedDir = document.documentElement.getAttribute('dir') || 'ltr';


  const setThemeSelectOnLoad = document.getElementById('set-theme');
  if (setThemeSelectOnLoad) setThemeSelectOnLoad.value = isLightDefault ? 'light' : 'dark';

  const setLangSelectOnLoad = document.getElementById('set-language');
  if (setLangSelectOnLoad) setLangSelectOnLoad.value = savedDir === 'rtl' ? 'ar' : 'en';

  const welcomeMsgOnLoad = document.getElementById('welcomeMsg');
  if (welcomeMsgOnLoad) {
    welcomeMsgOnLoad.textContent = savedDir === 'rtl' ? 'مرحباً بعودتك، المدير' : 'Welcome Back, Admin';
  }
  ...
  const htmlObserver = new MutationObserver((mutations) => {
    mutations.forEach((mutation) => {
      if (mutation.attributeName === 'dir') {
        ...
        if (setLangSelect) setLangSelect.value = dir === 'rtl' ? 'ar' : 'en';
        const welcomeMsg = document.getElementById('welcomeMsg');
        if (welcomeMsg) {
          welcomeMsg.textContent = dir === 'rtl' ? 'مرحباً بعودتك، المدير' : 'Welcome Back, Admin';
        }
      }
    });
  });
